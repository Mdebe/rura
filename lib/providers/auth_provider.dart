...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> _onAuthStateChanged(User? firebaseUser) async {
    // FIX: Prevent race condition with login()
    if (firebaseUser?.uid == _currentUser?.uid) return;
  ...
  Future<String?> login({
    ...
    try {
      ...
      // 2. IF NETWORK FAILED, TRY OFFLINE
      if (e.code == 'network-request-failed' || e.code == 'internal-error') {
        ...
        if (cached == null || cached.email != emailTrimmed) {
          return 'Offline login only available for last user. Connect to internet.';
        }

        // Verify password hash
        final storedHash = prefs.getString('pwd_hash_$lastUid');
      ...
      // 3. CATCH-ALL: Try offline as last resort
      final prefs = await SharedPreferences.getInstance();
      final lastUid = prefs.getString('last_uid');
      if (lastUid != null) {
        final cached = await DBHelper.instance.getUserByUid(lastUid);
  ...
  Future<String?> changePassword({
    ...
    try {
      ...
      // Update stored hash for offline login
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(
        'pwd_hash_${_firebaseUser!.uid}',
  ...
  Future<String?> deleteAccount(String password) async {
    ...
    try {
      ...
      await prefs.remove('pwd_hash_${_firebaseUser!.uid}');
